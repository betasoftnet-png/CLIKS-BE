...
CREATE TABLE IF NOT EXISTS money_trackers (
  ...
  if (dbType === 'postgres') {
    ...
    const pgAlters = [
      ...
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS is_online INTEGER DEFAULT 0;',
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS last_seen_at TEXT;',
      'ALTER TABLE users ADD COLUMN IF NOT EXISTS login_at TEXT;',
      ...
      'ALTER TABLE people ADD COLUMN IF NOT EXISTS role_type VARCHAR(100);',
      'ALTER TABLE people ADD COLUMN IF NOT EXISTS company VARCHAR(255);',
      'ALTER TABLE people ADD COLUMN IF NOT EXISTS contact_info TEXT;',
    ...
    const alterQueries = [
      ...
      'ALTER TABLE people ADD COLUMN role_type TEXT',
      'ALTER TABLE people ADD COLUMN company TEXT',
      'ALTER TABLE people ADD COLUMN contact_info TEXT',
      ...
      'ALTER TABLE gst_invoices ADD COLUMN goods_product_name TEXT',
      'ALTER TABLE gst_invoices ADD COLUMN goods_hsn_code TEXT',
      'ALTER TABLE gst_invoices ADD COLUMN goods_quantity REAL',
      ...
      'ALTER TABLE users ADD COLUMN login_at TEXT',
      'ALTER TABLE users ADD COLUMN receive_data INTEGER DEFAULT 1',
      'ALTER TABLE users ADD COLUMN receiveData INTEGER DEFAULT 1',
  ...
  try {
    ...
    const purchaseAlters = [
      'ALTER TABLE customer_purchase_history ADD COLUMN invoice_type TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN due_date TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN payment_mode TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN upi_id TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN bank_account_id TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN merchant_email TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN merchant_logo TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN customer_gstin TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN shipping_address TEXT',
      'ALTER TABLE customer_purchase_history ADD COLUMN subtotal REAL DEFAULT 0',
      'ALTER TABLE customer_purchase_history ADD COLUMN round_off REAL DEFAULT 0',
      'ALTER TABLE customer_purchase_history ADD COLUMN paid_amount REAL DEFAULT 0',
      'ALTER TABLE customer_purchase_history ADD COLUMN due_amount REAL DEFAULT 0',
      ...
      'ALTER TABLE customer_purchase_history ADD COLUMN sendPurchaseHistoryToCustomer INTEGER DEFAULT 1',

      'ALTER TABLE purchase_history ADD COLUMN invoice_type TEXT',
      'ALTER TABLE purchase_history ADD COLUMN due_date TEXT',
      'ALTER TABLE purchase_history ADD COLUMN payment_mode TEXT',
      'ALTER TABLE purchase_history ADD COLUMN upi_id TEXT',
      'ALTER TABLE purchase_history ADD COLUMN bank_account_id TEXT',
      'ALTER TABLE purchase_history ADD COLUMN merchant_email TEXT',
      'ALTER TABLE purchase_history ADD COLUMN merchant_logo TEXT',
      'ALTER TABLE purchase_history ADD COLUMN customer_gstin TEXT',
      'ALTER TABLE purchase_history ADD COLUMN shipping_address TEXT',
      'ALTER TABLE purchase_history ADD COLUMN subtotal REAL DEFAULT 0',
      'ALTER TABLE purchase_history ADD COLUMN round_off REAL DEFAULT 0',
      'ALTER TABLE purchase_history ADD COLUMN paid_amount REAL DEFAULT 0',
  ...
  try {
    ...
    `).run();
  // ── Capital Matrix / SME Deal Marketplace Migrations ──────────────────────
  try {
    const idType = dbType === 'postgres' ? 'SERIAL PRIMARY KEY' : 'INTEGER PRIMARY KEY AUTOINCREMENT';
    
    // 1. Users Schema Enhancements
    await db.prepare(`ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'FOUNDER'`).run().catch(() => {});
    await db.prepare(`ALTER TABLE users ADD COLUMN plan_type TEXT`).run().catch(() => {});
    await db.prepare(`ALTER TABLE users ADD COLUMN plan_status TEXT DEFAULT 'ACTIVE'`).run().catch(() => {});

    // 2. Venture Profiles Table
    await db.prepare(`
    ...
    await db.prepare(`
      CREATE TABLE IF NOT EXISTS pitches (
        ...
        goal_amount REAL DEFAULT 0 CHECK (goal_amount >= 0),
        equity_offered REAL DEFAULT 0 CHECK (equity_offered >= 0 AND equity_offered <= 100),
        deck_url TEXT,
        status TEXT DEFAULT 'PENDING_REVIEW' CHECK (status IN ('PENDING_REVIEW', 'ACCEPTED', 'REJECTED')),
        admin_remarks TEXT,
        created_at TEXT,
        updated_at TEXT,
        FOREIGN KEY(user_id) REFERENCES users(id)
    ...
    // Ensure columns on existing venture_pitches as well for backward compatibility
    await db.prepare(`ALTER TABLE venture_pitches ADD COLUMN status TEXT DEFAULT 'PENDING_REVIEW'`).run().catch(() => {});
    await db.prepare(`ALTER TABLE venture_pitches ADD COLUMN admin_remarks TEXT`).run().catch(() => {});

    // 4. User Unlocked Pitches Table (Quota Engine)
    await db.prepare(`
      CREATE TABLE IF NOT EXISTS user_unlocked_pitches (
        id ${idType},
    ...
    console.log('✅ Capital Matrix DB tables & migrations verified');
  } catch (capErr) {
    console.warn("⚠️ Error initializing Capital Matrix migrations:", capErr.message);
  }
